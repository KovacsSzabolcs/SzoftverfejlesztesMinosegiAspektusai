...
////
    @RequestMapping(value = "/todo/delete/id={id},todoid={todoid}", method = RequestMethod.GET)
    public String deleteToDo(@PathVariable("id") Long id, @PathVariable("todoid") Long todoid,Model model) throws NotFoundException {
  
    	
       
    	
        Story found = service.deleteToDo(id,todoid);
        model.addAttribute(MODEL_ATTRIBUTE, found);
        return VIEW_VIEW;
    
    }
    
    @RequestMapping(value = "/todo/random/{id}", method = RequestMethod.GET)
    public String random(@PathVariable("id") Long id, Model model) throws NotFoundException {
        ...
        StoryDTO testStory=new StoryDTO();
        testStory.setId(id);

        service.addToDo(testStory, temp);
    ...
    public String findAllToDos(Model model) {
		...
		System.out.println("--------------------------------------1");
		for (Story temp2 : service.findAll()) {
